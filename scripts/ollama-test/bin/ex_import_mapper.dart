...
void main(List<String> args) async {
  final existingExercisesfile = File(
      '/Users/lukeknoble/depot/open_fitness_tracker/scripts/ollama-test/foreignExNames.txt');
  List<String> exercises = await existingExercisesfile.readAsLines();

  final modelName = "qwen2.5:32b"; //nicest so far?
  // final modelName = "gemma2:27b";
  // final modelName = "llama3.1:latest";
  final mappingOutput = File(
    './ex_import_mapper/exercise_mapping_$modelName.txt',
  );
  final infoFile = File('./ex_import_mapper/info_$modelName.txt');
  var mappingFileWriter = mappingOutput.openWrite();
  var infoFileWriter = infoFile.openWrite();

  int numBadResponses = 0;
  for (int i = 0; i < exercises.length; i++) {
    var exName = exercises[i];
    print("processed: $i/${exercises.length}");

    String correspondingExercise = await querySender(exName, modelName);

    if (correspondingExercise.isEmpty) {
      correspondingExercise = "NO CORRESPONDING EX";
    } else if (!exList.contains(correspondingExercise)) {
      print("a New Ex Appeared: \n $correspondingExercise");
      infoFileWriter.writeln("$correspondingExercise\n");

      if (++numBadResponses > 4) {
        infoFileWriter.writeln("failed here.\n");
      } else if (i != 0) {
        //retry
        --i;
        continue;
    ...
    mappingFileWriter.write(correspondingExercise);
    mappingFileWriter.write('\n');
  }
  mappingFileWriter.write("]");
}

Future<String> querySender(String exerciseName, String modelName) async {
  //todo look into lowering temp and raising context
  //(queryParams: Map<String, dynamic>?{"junk", 2000});
  OllamaClient client = OllamaClient();

  String systemPrompt = """
      Below is a list of existing exercises. 
      You will be given a name of a single exercise and you will try to find the matching exercise from the list.
      It is OK to not be able to find a correponding exercise.
      ONLY reply with the NAME of the corresponding exercise.
  ...
  final generated = await client.generateCompletion(
    request: GenerateCompletionRequest(
        ...
        system: systemPrompt,
        prompt: exerciseName,
        options: RequestOptions(numCtx: 16000)),
  );

  print("$exerciseName -> ${generated.response!}");
